...
data_Y3$p <- predict(prox_pp_s1, newdata = data_Y3)
...
pci.pp.cox.pointest <- tidy(prox_pp_s2, conf.int = TRUE, exponentiate = TRUE)
saveRDS(pci.pp.cox.pointest, paste0(res_path,"pci.pp.cox.pointest.rds"))
...
boot.results <- lapply(1:num.boot, function(i){
  ...
  message("Starting PCI PP Cox bootstrap ", i, " (seed=", seed[i], ")")
  ...
  dat.boot$p <- predict(prox_pp_s1, newdata = dat.boot)
