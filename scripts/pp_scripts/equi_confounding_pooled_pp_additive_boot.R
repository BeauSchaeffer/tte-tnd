...
##----- Equi-Confounding Pooled ADDITIVE PP Bootstrap
##----- Per-protocol, no censoring weights
##----- last updated 2026-08-12
##-----
##----- Matching (subclass) nonparametric bootstrap for the additive marginal CIF.
##----- Kept line-for-line parallel to equi_confounding_pooled_pp_boot.R: the only
##----- differences are gaussian (identity-link) hazard models, the additive
##----- de-biasing lambda2(A=0)+[lambda1(A=1)-lambda1(A=0)], and clamping.
...
boot.results <- lapply(1:num.boot, function(i){
  ...
  t0 <- Sys.time()
  set.seed(seed[i])
  message("Starting EQC PP additive bootstrap ", i, " (seed=", seed[i], ")")
  ...
  dat.long$Y_pos <- ifelse(dat.long$Y3_pp_trunc==2 & dat.long$Y3_pp_t_trunc==dat.long$time_start, 1, 0)
  dat.long$Y_neg <- ifelse(dat.long$Y3_pp_trunc==1 & dat.long$Y3_pp_t_trunc==dat.long$time_start, 1, 0)
  dat.long$C     <- ifelse(dat.long$Y3_pp_trunc==0 & dat.long$Y3_pp_t_trunc==dat.long$time_start, 1, 0)
  ...
  eqc_pp_A0.long$hazard_pos <- predict(fit2, newdata=eqc_pp_A0.long)
  eqc_pp_A1.long$hazard_pos <- predict(fit2, newdata=eqc_pp_A1.long)
  eqc_pp_A0.long$hazard_neg <- predict(fit1, newdata=eqc_pp_A0.long)
  eqc_pp_A1.long$hazard_neg <- predict(fit1, newdata=eqc_pp_A1.long)
...
saveRDS(boot.long, paste0(res_path, "eqc.add.pp.boot.long.rds"))
