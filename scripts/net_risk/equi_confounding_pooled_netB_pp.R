...
##----- Equi-Confounding Analysis Pooled -- NET RISK, Option B
...
### fit1 is now the recurrent negative-testing hazard on {T2 > t} rather than
### a cause-specific first-test hazard. Same covariates, same time basis.
...
saveRDS(eqc_pooled_pp_fit1, paste0(res_path,"netB.eqc_pooled_pp_fit1.rds"))
...
saveRDS(eqc_pooled_pp_fit2, paste0(res_path,"netB.eqc_pooled_pp_fit2.rds")) 
...
### hazard does not enter the survival product at all -- it enters only
### through the equi-confounding correction to the positive hazard below.
...
eqc_pp_A0.long$survival_pos <- ave(eqc_pp_A0.long$pnoevent_pos_lag, eqc_pp_A0.long$fake_mrn, FUN=cumprod)
eqc_pp_A1.long$survival_pos <- ave(eqc_pp_A1.long$pnoevent_pos_lag, eqc_pp_A1.long$fake_mrn, FUN=cumprod)
eqc_pp_A0.long$survival_pos_c <- ave(eqc_pp_A0.long$pnoevent_pos_c_lag, eqc_pp_A0.long$fake_mrn, FUN=cumprod)
...
netB.eqc.pp.risk.pointest <- tibble(
...
saveRDS(netB.eqc.pp.risk.pointest, paste0(res_path, "netB.eqc.pp.risk.pointest.rds"))
