...
### Negative-test weeks inside each person's PP risk set. This does not depend
### on the resample, so it is built once and joined on (fake_mrn, time_start)
### inside the loop -- fake_mrn repeats across drawn copies of a person, and
### every copy correctly receives the same negative weeks.
...
saveRDS(boot.long, paste0(res_path, "netB.eqc.pp.boot.long.rds"))
